...
<p style="text-align:center; height:40px; margin:0px auto;"><strong>'.$kt['nama_produk'].'</strong></p><p style="text-align:center; margin:0px auto;"><img src="'.base_url().'asset/produk/'.$kt['gbr_kecil'].'" width="100" /><br />
Rp. '.number_format($kt['harga'],2,',','.').'<br>
Stok : '.$tss.'<div style="width:152px; margin:0px auto; padding:0px;">
<a href="'.base_url().'index.php/awal/detail/'.$kt['kode_produk'].'-'.$link.'" class="vtip" title="'.$kt['nama_produk'].' - Harga Rp.'.number_format($kt['harga'],2,',','.').'"><img src="'.base_url().'asset/images/bar-detail.png" border=0 style="float:right;" /></a></div></p></div></form>';
}
}
else{
